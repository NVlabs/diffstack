...
import numpy as np
import pandas as pd
from diffstack.modules.predictors.trajectron_utils.environment import DoubleHeaderNumpyArray
...
class Node(object):
    def __init__(self, node_type, node_id, data, length=None, width=None, height=None, first_timestep=0,
                 is_robot=False, description="", frequency_multiplier=1, non_aug_node=None, extra_data=None):
        self.type = node_type
        self.id = node_id
        self.length = length
        self.width = width
    ...
    def get_lane_points(self, tr_scene, padding=np.nan, num_lane_points=16) -> np.ndarray:
        """
        :param tr_scene: The timestep range (inklusive).
        """
        if tr_scene.size == 1:
        ...
        length = tr_scene[1] - tr_scene[0] + 1  # tr is inclusive
        tr, paddingl, paddingu = self.scene_ts_to_node_ts(tr_scene)
        data_array = self.extra_data['lane_points'][tr[0]:tr[1] + 1]
        padded_data_array = np.full((length, data_array.shape[1], data_array.shape[2]), fill_value=padding)
        padded_data_array[paddingl:length - paddingu] = data_array
        # extend to fixed num lane points
        if num_lane_points is not None:
            if padded_data_array.shape[1] == 0:
                padded_data_array = np.full((length, num_lane_points, 3), fill_value=padding)
            elif padded_data_array.shape[1] < num_lane_points:
                pad = np.repeat(padded_data_array[:, -1:], num_lane_points-padded_data_array.shape[1], axis=1)
                padded_data_array = np.concatenate((padded_data_array, pad), axis=1)
            else:
                padded_data_array = padded_data_array[:, :num_lane_points]
        return padded_data_array
