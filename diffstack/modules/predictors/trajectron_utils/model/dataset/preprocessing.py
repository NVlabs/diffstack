...
def collate(batch):
    ...
    if elem is None:
        return None
    elif isinstance(elem, str) or isinstance(elem, batchable_list) or isinstance(elem, batchable_nonuniform_tensor):
...
def get_relevant_lanes_np(nusc_map, scene_offset, position, yaw, vel, dt):
    ...
    # TODO we should get the lanes from three anchor points: 
    # - max deceleration
    # - [constant speed]
    # - max acceleration
...
def pred_state_to_plan_state(pred_state):
    ...
    # TODO some dataset versions after v6 set USE_KALMAN_FILTER=True which computes acceleration differently.
...
def get_node_timestep_data(env, scene, t, node, state, pred_state,
                           ...
                           nusc_maps, scene_graph=None,
                           is_closed_loop=False, closed_loop_ego_hist=None):
    """
    ...
    plan_vehicle_state_dict = {
        'position': ['x', 'y'], 'heading': ['°'], 'velocity': ['norm']}  
    plan_vehicle_features_dict = {        
        'heading': ['d°'], 'acceleration': ['norm2'],   # control
        'lane': ['x', 'y', '°'], 'projected': ['x', 'y'],    # lane and lane-projected states
        'control_traj_dh': ["t"+str(i) for i in range(max_ft+1)],  # future fitted controls
    ...
    plan_pedestrian_state_dict = {'position': ['x', 'y'],}  

    # Filter fields not in data
    state = {nk: {k: v for k, v in ndict.items() if k != "augment"} for nk, ndict in state.items()}
        ...
        for edge_type in edge_types:
            ...
            for n_i, connected_node in enumerate(connected_nodes):
                ...
                if is_closed_loop and connected_node.is_robot:
                    ...
                    logged_robot_data = torch.tensor(neighbor_state_np, dtype=torch.float)
                    if closed_loop_ego_hist is not None:
                        assert neighbor_state_np.shape[0] <= closed_loop_ego_hist.shape[0]
                        assert neighbor_state_np.shape[1] == closed_loop_ego_hist.shape[1]
                ...
                if is_closed_loop:
                    neighbors_data_not_st[edge_type].append(torch.tensor(neighbor_state_np, dtype=torch.float))
                ...
                if edge_type[1] == "VEHICLE": 
                    ...
                    try:                                       
                        neighbor_future_features_np = np.concatenate((
                            # x, y, orient, vel
                            connected_node.get(timestep_range_plan, 
                                            plan_vehicle_state_dict,
                                            padding=np.nan), 
                    ...
                    if is_closed_loop and closed_loop_ego_hist is not None and connected_node.is_robot: 
                        neighbor_future_features_np[0, :6] = pred_state_to_plan_state(closed_loop_ego_hist[-1])
                            
                    # Add lane points
                    lane_ref_points = connected_node.get_lane_points(timestep_range_plan, padding=np.nan, num_lane_points=16)
                    neighbor_future_features_np = np.concatenate((neighbor_future_features_np, lane_ref_points.reshape((lane_ref_points.shape[0], 16*3))), axis=1)
                    ...
                    xy_first = connected_node.data[0, {'position': ['x', 'y']}].astype(np.float32)
                    xy_last = connected_node.data[connected_node.last_timestep-connected_node.first_timestep, {'position': ['x', 'y']}].astype(np.float32) 
                    is_neighbor_parked[edge_type].append(np.square(xy_first - xy_last).sum(0) < 1.**2)

                elif edge_type[1] == "PEDESTRIAN":
                    neighbor_future_features_np = connected_node.get(timestep_range_plan,
                                                                plan_pedestrian_state_dict,
                                                                padding=0.0)  # x, y | lane x y heading | projected x y
                else:
                    raise ValueError("Unknown type {}".format(edge_type[1]))                                                                
                neighbor_future_features = torch.tensor(neighbor_future_features_np, dtype=torch.float)
                neighbors_future_data[edge_type].append(neighbor_future_features)

                assert hyperparams['dt'] == scene.dt  # we will rely on this hyperparam for delta_t, make sure its correct
            ...
            if edge_type[1] == 'VEHICLE':
                ...
                for n_i in range(len(vehicle_future_f)):
                    ...
                    dist = torch.square(y_t - vehicle_future_f[n_i][1:, :2])  # [1:] exclude current state for vehicle future
                    dist = dist.sum(dim=-1).amin(dim=-1)  # sum over states, min over time
                    inds.append(n_i)
                    dists.append(dist)
                    
                if dists:
                    plan_i = inds[torch.argmin(torch.stack(dists))]  # neighbor that gets closest to current node
                else:
                    # No neighbors or all futures are incomplete
                    plan_i = -1

                # Robot index, filter incomplete futures, controls or missing lanes
                if robot_neighbor >= 0 and torch.isnan(vehicle_future_f[robot_neighbor][:, :(4+2+3)]).any():
                    robot_i = -1
                else:
                ...
                if plan_i >= 0:    
                    ...
                    pos_xy_goal, yaw_goal, vel_goal = np.split(x_goal_np, (2, 3), axis=-1)                
                    relevant_lanes, relevant_lane_tokens, relevant_lane_arclines = get_relevant_lanes_np(nusc_map, scene_offset_np, pos_xy_goal, yaw_goal, vel_goal, hyperparams['dt'])
                else:
                    relevant_lanes = []
                ...
                plan_data = batchable_dict(
                    ...
                    most_relevant_nearby_lanes=batchable_list([torch.from_numpy(pts).float() for pts in relevant_lanes]),
                    most_relevant_nearby_lane_tokens=batchable_list(relevant_lane_tokens),
    ...
    data_tuple = (first_history_index, x_t, y_t, x_st_t, y_st_t, neighbors_data_st,
            neighbors_edge_value, robot_traj_st_t, map_tuple, neighbors_future_data, plan_data)
    if is_closed_loop:
