...
from diffstack.modules.predictors.trajectron_utils.model.components import *
from diffstack.modules.predictors.trajectron_utils.model.model_utils import *
import diffstack.modules.predictors.trajectron_utils.model.dynamics as dynamic_module
from diffstack.modules.predictors.trajectron_utils.environment.scene_graph import DirectedEdge
...
class MultimodalGenerativeCVAE(nn.Module):
    def __init__(self,
        ...
        self.node_type_state_lengths = {k: 
                int(np.sum([len(entity_dims) for entity_dims in self.state[k].values()]))
                for k in self.state.keys()}
        self.state_length = self.node_type_state_lengths[node_type]
        ...
        self.dynamic = dynamic_class(self.env.dt, dyn_limits, device,
    ...
    def obtain_encoded_tensors(self,
                               ...
                               robot,
                               map) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, 
    ...
    def encode_edge(self,
        ...
        for i, neighbor_states in enumerate(neighbors):  # Get neighbors for timestep in batch
            if len(neighbor_states) == 0:  # There are no neighbors for edge type # TODO necessary?
                neighbor_state_length = self.node_type_state_lengths[edge_type[1]]
    ...
    def decoder(self, mode, x, x_nr_t, y, y_r, n_s_t0, z, labels, prediction_horizon, num_samples, ret_dist=False):
        """
        Decoder of the CVAE.
    ...
    def train_loss(self,
        ...
        log_p_y_xz, y_dist = self.decoder(mode, x, x_nr_t, y, y_r, n_s_t0, z,
        ...
        if loss_weights is not None:
            # Weighted sum over batch. Weights are expected to sum to 1.
            log_likelihood = torch.sum(log_p_y_xz_mean * loss_weights, dim=0)
        else:
    ...
    def eval_loss(self,
        ...
        z, _ = self.encoder(mode, x, y_e)  # [k, nbs, N*K]
    ...
    def predict(self,
        ...
        y_dist, our_sampled_future = self.p_y_xz(mode, x, x_nr_t, y_r, n_s_t0, z,
                                                 ...
                                                 gmm_mode)
        if output_extra:
            return y_dist, our_sampled_future, (x, )
        if output_dists:
