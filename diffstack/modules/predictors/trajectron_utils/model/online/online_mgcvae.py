...
class OnlineMultimodalGenerativeCVAE(MultimodalGenerativeCVAE):
    def __init__(self,
        ...
        super(OnlineMultimodalGenerativeCVAE, self).__init__(env, node.type, model_registrar, hyperparams, device, edge_types=[])
    ...
    def obtain_encoded_tensors(self,
        ...
        if self.hyperparams['incl_robot_node'] and self.robot is not None:
            node_state = torch.zeros((robot_present_and_future.shape[-1], ), dtype=torch.float, device=self.device)
            node_state[:our_inputs.shape[1]] = our_inputs[0]
            robot_present_and_future_st = get_relative_robot_traj(self.env, self.state,
                                                                  node_state, robot_present_and_future,
