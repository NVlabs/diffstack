...
class DataFormat(object):
    ...
    def satisfied_by(self, data_dict: Dict[str, Any]) -> bool:
        # TODO(pkarkus) check types / shapes not only namess
...
class Module(torch.nn.Module):
    ...
    @property
    def name(self) -> str: self.__class__.__name__
    ...
    def apply_input_mappings(self, inputs: Dict, run_mode: RunMode, allow_partial: bool = False):
        mapped_inputs = {}
    ...
    # The user is expected to interact with a module through the train/validate/infer methods. 
    # Childrens of this abstract class should either override the train/validate/infer 
    # methods or implement a _run_forward function.
    def train(self, inputs: Dict) -> Dict:
        return self._run_forward(inputs, RunMode.TRAIN)
    
    def validate(self, inputs: Dict) -> Dict:
        return self._run_forward(inputs, RunMode.VALIDATE)
    
    def infer(self, inputs: Dict) -> Dict:
        return self._run_forward(inputs, RunMode.INFER)

    def _run_forward(self, inputs: Dict, run_mode: RunMode) -> Dict:
        raise NotImplementedError
...
class ModuleSequence(Module):
    ...
    Example:
        ...
        output_dict = stack.train(input_dict)
    ...
    """    
    @property
    def input_format(self) -> DataFormat: self.components[0].input_format

    @property
    def output_format(self) -> DataFormat: self.components[-1].input_format
    ...
    def __init__(
        ...
        super().__init__(model_registrar, hyperparams, log_writer, device, input_mappings=input_mappings)
        self.components = components

        if 'input' in self.components:
            raise ValueError('Name "input" is reserved for the overall input to the module sequence.')
        # TODO check there is no "." in the name
    ...
    def validate_interfaces(self, desired_input: DataFormat) -> bool:
        ...
        for component in self.components.values():
            if component.input_format.satisfied_by(data_dict):
                ...
                missing_inputs = [x for x in component.input_format.required_elements if x not in data_dict]
                raise ValueError(f"Component '{component.name}' missing input(s): {missing_inputs}")

        return data_dict
    
    def sequence_components(self, inputs: Dict, run_mode: RunMode):
        ...
        for name, component in self.components.items():
            ...
            if run_mode ==  RunMode.TRAIN:
                output = component.train(inputs)
            elif run_mode == RunMode.VALIDATE:
                output = component.validate(inputs)
            elif run_mode == RunMode.INFER:
                output = component.infer(inputs)
            else:
    ...
    def dry_run(
        ...
        if run_mode is None:
            for run_mode in RunMode:
                issues.extend(self.dry_run(input_keys, run_mode, check_output, raise_error=False))
            ...
            component: Module
            for name, component in self.components.items():
                inputs = component.apply_input_mappings(next_inputs, run_mode, allow_partial=True)
                input_format = component.input_format.for_run_mode(run_mode)
                if not input_format.satisfied_by(inputs):
                    issues.append(f"Component {name}({run_mode.name}): \n  Required inputs: {list(input_format)}\n  Provided inputs: {list(inputs.keys())}")

                output = {k: None for k in component.output_format.for_run_mode(run_mode)}
                # Add to all_outputs 
                all_outputs.update({f"{name}.{k}": v for k, v in output.items()})
                # Construct input from all_outputs and the previous outputs.
                next_inputs = {**all_outputs, **output}      
    ...
    def _run_forward(self, inputs: Dict, run_mode: RunMode) -> Dict:
        raise self.sequence_components(inputs, run_mode=run_mode)
