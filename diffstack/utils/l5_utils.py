...
def raw_to_features(batch_raw):
    ...
    pos = batch_raw["hist_pos"]
    vel = batch_raw["history_velocities"]
    yaw = batch_raw["hist_yaw"]
    mask = batch_raw["hist_mask"]
...
def raw_to_states(batch_raw):
    ...
    vel = batch_raw["history_velocities"]
    yaw = batch_raw["hist_yaw"]
    avail_mask = batch_raw["hist_mask"]
...
def batch_to_raw_ego(data_batch, step_time):
    batch_size = data_batch["hist_pos"].shape[0]
    raw_type = torch.ones(batch_size).type(torch.int64).to(data_batch["hist_pos"].device)  # [B, T]
    raw_type = raw_type * 3  # index for type PERCEPTION_LABEL_CAR

    src_pos = torch.flip(data_batch["hist_pos"], dims=[-2])
    src_yaw = torch.flip(data_batch["hist_yaw"], dims=[-2])
    src_mask = torch.flip(data_batch["hist_mask"], dims=[-1]).bool()

    src_vel = dynamics.Unicycle(step_time).calculate_vel(pos=src_pos, yaw=src_yaw, mask=src_mask)
...
def batch_to_vectorized_feature(data_batch, dyn_list, step_time, algo_config):
    device = data_batch["hist_pos"].device
    raw_type = torch.cat(
    ...
    src_pos = torch.cat(
        (
            ...
            data_batch["all_other_agents_hist_pos"],
        ),
        dim=1,
    )
    ...
    src_yaw = torch.cat(
        (
            ...
            data_batch["all_other_agents_hist_yaw"],
        ),
        dim=1,
    )
...
def batch_to_raw_all_agents(data_batch, step_time):
    ...
    # estimate velocity
    src_vel = dynamics.Unicycle(step_time).calculate_vel(src_pos, src_yaw, src_mask)
...
def get_edges_from_batch(data_batch, ego_predictions=None, all_predictions=None):
    ...
    if ego_predictions is not None:
        targets_all["fut_pos"] [:, 0, :, :] = ego_predictions["positions"]
        targets_all["fut_yaw"][:, 0, :, :] = ego_predictions["yaws"]
    elif all_predictions is not None:
        targets_all["fut_pos"] = all_predictions["positions"]
        targets_all["fut_yaw"] = all_predictions["yaws"]
    else:
    ...
    pred_edges = generate_edges(
        ...
        pos_pred=targets_all["fut_pos"],
        yaw_pred=targets_all["fut_yaw"]
    )
    return pred_edges
...
def get_current_states(batch: dict, dyn_type: dynamics.DynType) -> torch.Tensor:
    ...
    if dyn_type == dynamics.DynType.BICYCLE:
        ...
        current_states[:, [4]] = (batch["hist_yaw"][:, 0] - batch["hist_yaw"][:, 1]).abs()
...
def get_current_states_all_agents(batch: dict, step_time, dyn_type: dynamics.DynType) -> torch.Tensor:
    ...
    if dyn_type == dynamics.DynType.BICYCLE:
        ...
        current_states[:, :, :2] = state_all["hist_pos"][:, :, 0]
        current_states[:, :, 3] = state_all["curr_speed"].abs()
        current_states[:, :, [4]] = (state_all["hist_yaw"][:, :, 0] - state_all["hist_yaw"][:, :, 1]).abs()
        ...
        current_states[:,:,3:] = state_all["hist_yaw"][:,:,0]
    return current_states
...
def get_modality_shapes(cfg: ExperimentConfig):
    ...
    num_channels = (cfg.stack.history_num_frames + 1) * 2 + 3
