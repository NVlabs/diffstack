...
def recursive_dict_list_tuple_apply(x, type_func_dict, ignore_if_unspecified=False):
    ...
    if isinstance(x, (dict, collections.OrderedDict, nn.ParameterDict)):
        ...
        for k, v in x.items():
            new_x[k] = recursive_dict_list_tuple_apply(
                v, type_func_dict, ignore_if_unspecified
            )
        return new_x
    elif isinstance(x, (list, tuple, nn.ParameterList)):
        ret = [
            recursive_dict_list_tuple_apply(v, type_func_dict, ignore_if_unspecified)
            for v in x
        ]
...
def pad_sequence_single(seq, padding, batched=False, pad_same=False, pad_values=0.0):
    """
    ...
    if padding[1] > 0:
        if batched:
            pad = (
                seq[:, [-1]] if pad_same else ones_like_func(seq[:, [-1]]) * pad_values
            )
        else:
...
def pad_sequence(seq, padding, batched=False, pad_same=False, pad_values=0.0):
    """
...
def block_diag_from_cat(x):
    ...
    if x.ndim == 3:
        ...
        Id = torch.eye(M, device=x.device)
        slices = [
            torch.kron(Id[i], x[:, i].reshape(-1, m)).reshape(bs, n, m * M)
            for i in range(M)
        ]
        mat = torch.cat(slices, 1)
        return mat
...
def recursive_mean(x):
    # x is a list of dict, potentially multi-level, with torch.Tensor as values, we want to compute the mean.
    output = dict()
    for k, v in x[0].items():
        if isinstance(v, dict):
            output[k] = recursive_mean([xi[k] for xi in x])
        elif isinstance(v, torch.Tensor):
            output[k] = torch.stack([xi[k] for xi in x], dim=0).mean(dim=0)
        else:
            raise ValueError("value must be torch.Tensor or dict")
    return output
