...
from diffstack.sim.TBSIM.base import BatchedEnv, BaseEnv
import diffstack.utils.tensor_utils as TensorUtils
from diffstack.utils.timer import Timers
from diffstack.sim.env_builders import EnvNuscBuilder
from diffstack.policies.wrappers import RolloutWrapper, Pos2YawWrapper
import diffstack.utils.geometry_utils as GeoUtils
from diffstack.utils.trajdata_utils import parse_trajdata_batch
from diffstack.utils.geometry_utils import VEH_VEH_collision
...
def collision_check(agents_posyaw, new_posyaw, agents_extent, new_extent):
    ...
    dis = VEH_VEH_collision(
        new_posyaw_tiled, agents_posyaw, new_extent_tiled, agents_extent
    )
    return dis
...
def random_placing_neighbors(simscene, num_neighbors, coll_check=True):
    ...
    for i in range(num_neighbors):
        ...
        newagent_type = 1
        newagent_extent = np.array([4.0, 2.5, 2.0])
        if init_modes[i] == 0:
            # in front of the ego vehicle
            newagent_state = np.array([[offset_x, 0, 0]]).repeat(T, 0)
            vel = np.clip(ego_vel - 2.0 + np.random.randn() * v_sigma, 0.0, 40.0)
        elif init_modes[i] == 1:
            ...
            vel = np.clip(ego_vel + np.random.randn() * v_sigma, 0.0, 40.0)
        elif init_modes[i] == 3:
            ...
            vel = np.clip(ego_vel - 4.0 + np.random.randn() * v_sigma, 0.0, 40.0)

        newagent_state[:, 0] += np.arange(-T + 1, 1) * dt * vel

        add_flag = True
        new_pos_global = GeoUtils.batch_nd_transform_points_np(
            newagent_state[:, :2], obs["world_from_agent"][0]
        )
        new_yaw_global = newagent_state[:, 2:] + obs["world_yaw"][0]
        newagent_state_global = np.hstack((new_pos_global, new_yaw_global))
        if coll_check:
            if "centroid" in obs:
                agents_pos_global = obs["centroid"]
            else:
                agents_pos_global = GeoUtils.batch_nd_transform_points_np(
                    obs["history_positions"][:, -1], obs["world_from_agent"]
                )
            agents_yaw_global = obs["world_yaw"]
            agents_posyaw = np.hstack(
                (agents_pos_global, agents_yaw_global[:, np.newaxis])
            )
            new_posyaw = np.hstack((new_pos_global[0], new_yaw_global[0]))

            dis = collision_check(
                agents_posyaw, new_posyaw, obs["extent"], newagent_extent
            )
            if dis.min() < 2.0:
                add_flag = False
...
def random_initial_adjust_plan(env, adjust_recipe):
    ...
    for simscene in env._current_scenes:
        adjust_plan[simscene.scene.name] = dict(
            remove_existing_neighbors=dict(
                flag=adjust_recipe["remove_existing_neighbors"], executed=False
            ),
            agents=random_placing_neighbors(
                simscene, adjust_recipe["initial_num_neighbors"]
            ),
        )
        ...
        while not done:
            ...
            with timers.timed("to_torch"):
                if obs_to_torch:
                    ...
                    obs_torch = TensorUtils.to_torch(
                        obs, device=device, ignore_if_unspecified=True
                    )
                else:
                    obs_torch = obs
...
class RolloutCallback(pl.Callback):
    ...
    def _get_env(self, device):
        ...
        if self._eval_cfg.env == "nusc":
            env_builder = EnvNuscBuilder(
                eval_config=self._eval_cfg, exp_config=self._exp_cfg, device=device
            )

        else:
            raise NotImplementedError(
                "{} is not a valid env".format(self._eval_cfg.env)
            )
    ...
    def _get_policy(self, pl_module: pl.LightningModule):
        ...
        composer = composer_class(
            self._eval_cfg, pl_module.device, ckpt_root_dir=self._eval_cfg.ckpt_root_dir
        )
    ...
    def _run_rollout(self, pl_module: pl.LightningModule, global_step: int):
        ...
        while scene_i < len(eval_scenes):
            scene_indices = eval_scenes[
                scene_i : scene_i + self._eval_cfg.num_scenes_per_batch
            ]
            ...
            if result_stats is None:
                ...
                for k in stats:
                    result_stats[k] = np.concatenate(
                        [result_stats[k], stats[k]], axis=0
                    )
            ...
            if self._save_video:
                for ei, episode_rendering in enumerate(renderings):
                    for i, scene_images in enumerate(episode_rendering):
                        video_fn = "{}_{}_{}.mp4".format(
                            global_step, info["scene_index"][i], ei
                        )

                        writer = get_writer(
                            os.path.join(self._video_dir, video_fn), fps=10
                        )
                        print(
                            "video to {}".format(
                                os.path.join(self._video_dir, video_fn)
                            )
                        )
