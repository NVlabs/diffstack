...
from diffstack.utils.geometry_utils import transform_points_tensor, calc_distance_map
from diffstack.utils.l5_utils import get_last_available_index
from diffstack.utils.loss_utils import goal_reaching_loss, trajectory_loss, collision_loss
...
def get_spatial_goal_supervision(data_batch):
    ...
    goal_index = get_last_available_index(
        data_batch["fut_mask"])[:, None, None]

    # gather by goal index
    goal_pos_agent = torch.gather(
        data_batch["fut_pos"],  # [B, T, 2]
        dim=1,
        index=goal_index.expand(-1, 1,
                                data_batch["fut_pos"].shape[-1])
    )  # [B, 1, 2]

    goal_yaw_agent = torch.gather(
        data_batch["fut_yaw"],  # [B, T, 1]
        dim=1,
        index=goal_index.expand(-1, 1, data_batch["fut_yaw"].shape[-1])
    )  # [B, 1, 1]
...
def optimize_trajectories(
    ...
    for oidx in range(num_optim_iterations):
        def closure():
            ...
            x = dynamics_model.forward_dynamics(
                x0=init_x,
                u=curr_u,
            )
            pos = dynamics_model.state2pos(x)
            yaw = dynamics_model.state2yaw(x)
    ...
    final_raw_trajs = dynamics_model.forward_dynamics(
        x0=init_x,
        u=curr_u,
    )
    final_pos = dynamics_model.state2pos(final_raw_trajs)
    final_yaw = dynamics_model.state2yaw(final_raw_trajs)
