...
from trajdata.utils.arr_utils import batch_nd_transform_points_angles_np, angle_wrap, batch_proj
...
class LanesList(list, CustomCollateData):
    ...
    def __collate__(elements: list) -> any:
        return LanesList([[torch.as_tensor(pts) for pts in lanelist] for lanelist in elements])
...
def get_lane_reference_points_from_polylines(global_xyh: np.ndarray, lane_polylines_xyh: Iterable[np.ndarray], max_num_points: int = 16, resolution_meters: float = 1.0):
    ...
    heading_diff_mat = np.minimum(
        ...
        np.abs(angle_wrap(heading_diff_mat + np.pi)))  # opposite direction
...
def get_lane_projection_points(
    ...
    ego_xyh_world = batch_nd_transform_points_angles_np(ego_xyhv[:, :3], world_from_agent_tf)

    # trajlen = ego_xyh_world.shape[0]
    # if trajlen == 0:
    #     lane_points_xyh_world = []
    # else:
    ego_xyz_world = np.concatenate((ego_xyh_world[:, :2], np.zeros_like(ego_xyh_world[:, :1])), axis=-1)
    ...
    lane_points_xyh_world = get_lane_reference_points_from_polylines(ego_xyh_world, lane_polylines_xyh, max_num_points=1)
    ...
    if lane_points_xyh_world.shape[1] == 0:
        ...
        lane_points_xyh = batch_nd_transform_points_angles_np(lane_points_xyh_world[..., :3], agent_from_world_tf[None])

        # Project state to lane
        # TODO this is based on a single lane point and its heading. Instead we should use lane segment. 
        # Get the two neighboring segments, project to both and choose shorter.
        # TODO in kdtree we whould interpolate polylines to a max_distance of e.g. 1m 
        lane_projection_points = lane_frenet_features_simple(ego_xyhv[..., :3], lane_points_xyh[:, :])   
...
def get_goal_lanes(
    ...
    world_from_agent_tf = np.linalg.inv(agent_from_world_tf)
    goal_xyh_world = batch_nd_transform_points_angles_np(goal_xyhv[:3], world_from_agent_tf)

    # Find lanes in range
    goal_xyz_world = np.concatenate((goal_xyh_world[:2], np.zeros_like(goal_xyh_world[:1])), axis=-1)
    ...
    for lane_xyh in near_lanes_xyh_world:
        ...
        if abs(dpsi[0]) < max_heading_delta and np.min(np.abs(delta_y)) < max_lateral_dist:
            lanes_xyh_world.append(lane_xyh)

    # World to agent coordinates
    goal_lanes = [batch_nd_transform_points_angles_np(
        lane_xyh, agent_from_world_tf[None]) for lane_xyh in lanes_xyh_world]
